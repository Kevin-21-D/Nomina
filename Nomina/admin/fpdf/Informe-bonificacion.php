...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(130, 10, 'Tipo de Bonificacion', 1, 0, 'C', !$this->fill);
        $this->Cell(42, 10, 'Monto', 1, 1, 'C', $this->fill);
...
$sql = "SELECT * FROM bonificaciones WHERE 1";
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $sql .= " AND (IDBonificacion LIKE '%$search%' OR TipoBonificacion LIKE '%$search%')";
}
...
if ($result = mysqli_query($link, $sql)) {
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            $pdf->Cell(18, 10, $row['IDBonificacion'], 1, 0, 'C', $pdf->fill);
            $pdf->Cell(130, 10, utf8_decode($row['TipoBonificacion']), 1, 0, 'C', !$pdf->fill);
            // Centrar horizontalmente el contenido en la celda
            $pdf->Cell(42, 10, '$' . $row['MontoBonificacion'], 1, 1, 'C', $pdf->fill);
        ...
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(0, 10, 'No se encontraron bonificaciones.', 0, 1);
    }
} else {
    $pdf->SetFont('Arial', '', 12);
